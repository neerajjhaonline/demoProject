...
<div class="container">
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                ...
                <div class="panel-heading">Create Indexing</div>
                <div class="panel-body">
                    ...
                    <form class="form-horizontal" role="form" method="POST" action="{{ route('auditing.store') }}" autocomplete="off" id="auditing">
                        {{ csrf_field() }}
                        
                        <input type="hidden" name="publishing_id" value="{{$recId->id or ''}}">
                        <input type="hidden" name="priorityHrs" value="{{$indexing->tat->tat_time or ''}}">
                        <?php date_default_timezone_set('Asia/Kolkata'); ?>

                        <input type="hidden" name="audit_start_dt" value="{{date('Y-m-d H:i:s')}}">
                            ...
                            <div class="col-md-6">
                                <select name="publish_by" class="form-control">
                                  ...
                                  @foreach(App\Model\UserAccess::all() as $access)
                                    <option value="{{$access->id}}" {{($access->id == $recId->publish_by ||  $access->id == old('audit_by')?'selected':'')}}>{{$access->name}}</option>
                                  @endforeach  
                                </select>
                            ...
                            <div class="col-md-6">
                                <select name="request_id" class="form-control">
                                  ...
                                  @foreach(App\Model\RequestType::all() as $rq)
                                    <option value="{{$rq->id}}" {{($rq->id == $indexing->request_id || $rq->id == old('request_id')?'selected':'')}}>{{$rq->name}}</option>
                                  @endforeach  
                                </select>
 ...
 -->
                            ...
                            <div class="col-md-6">
                                <select name="status_id" class="form-control">
                                  ...
                                  @foreach(App\Model\Status::all() as $status)
                                    <option value="{{$status->id}}" {{($status->id == old('status_id') || $status->id == $recId->status_id ?'selected':'')}} >{{$status->status_name}}</option>
                                  @endforeach  
                                </select>
                            ...
                            <div class="col-md-6">
                                <select name="rfiType_id" class="form-control">
                                  ...
                                  @foreach(App\Model\RfiType::all() as $rfi)
                                    <option value="{{$rfi->id}}" {{($rfi->id == $recId->rfiType_id || $rfi->id == old('rfiType_id')?'selected':'')}} >{{$rfi->rfiType_name}}</option>
                                  @endforeach  
                                </select>
                                @if ($errors->has('region_id'))
                                    <span class="help-block">
                        ...
                        <div class="form-group{{ $errors->has('rfi_description') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <textarea id="rfi_description" class="form-control" name="rfi_description">{{$recId->rfi_description or old('rfi_description')}}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('query_raised_by') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <select name="query_raised_by" class="form-control">
                                  <option value="">Select</option>
                                  @foreach(App\Model\UserAccess::all() as $access)
                                    <option value="{{$access->id}}" {{($recId->query_raised_by == $access->id || $rfi->id == old('query_raised_by')?'selected':'')}}>{{$access->name}}</option>
                                  @endforeach  
                                </select>
                     ...
                     <div class="form-group{{ $errors->has('markError') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input type="checkbox" value="1" name="markError" {{ (old('markError') == '1' ? 'checked':'')}} id="markError">
                    ...
                    <div id="correctionBlock" @if($indexing->requestType->name !== 'COR') style="display:none" @endif><!-- if Indexing is COR then correction ====== this block starts here -->
                            ...
                            <div class="col-md-6">
                                <select name="error_cat_id" class="form-control">
                                  ...
                                  @foreach(App\Model\ErrorCat::all() as $errCat)
                                    <option value="{{$errCat->id}}" {{($errCat->id == $recId->error_cat_id || $errCat->id == old('error_cat_id')?'selected':'')}} >{{$errCat->name}}</option>
                                  @endforeach  
                                </select>
                            ...
                            <div class="col-md-6">
                                <select name="error_type_id" class="form-control">
                                  ...
                                  @foreach(App\Model\ErrorType::all() as $errType)
                                    <option value="{{$errType->id}}" {{($errType->id == $recId->error_type_id || $errType->id == old('error_type_id')?'selected':'')}}>{{$errType->name}}</option>
                                  @endforeach  
                                </select>
                        ...
                        <div class="form-group{{ $errors->has('root_cause') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <textarea id="root_cause" class="form-control" name="root_cause">{{$recId->root_cause or old('root_cause')}}</textarea>
                        ...
                        <div class="form-group{{ $errors->has('correction') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <textarea id="correction" class="form-control" name="correction">{{$recId->correction or old('correction')}}</textarea>
                        ...
                        <div class="form-group{{ $errors->has('corrective_action') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <textarea id="corrective_action" class="form-control" name="corrective_action">{{$recId->corrective_action or old('corrective_action')}}</textarea>
                         ...
                         <div class="form-group{{ $errors->has('preventive_action') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <textarea id="preventive_action" class="form-control" name="preventive_action">{{$recId->preventive_action or old('preventive_action')}}</textarea>
                        ...
                        <div class="form-group{{ $errors->has('proposed_comp_dt') ? ' has-error' : '' }}">
                          ...
                          <div class="col-md-6">
                              <div class="input-group date">
                                <input name="proposed_comp_dt" data-format="MM/dd/yyyy HH:mm:ss PP" type="text" class="form-control" id="proposed_comp_dt" value="{{old('proposed_comp_dt')}}"></input>
                                <span class="input-group-addon add-on">
                                  <i data-time-icon="fa fa-clock-o" data-date-icon="fa fa-calendar">
                                  </i>
                                </span>
                        ...
                        <div class="form-group{{ $errors->has('proposed_act_dt') ? ' has-error' : '' }}">
                          ...
                          <div class="col-md-6">
                              <div class="input-group date">
                                <input name="proposed_act_dt" data-format="MM/dd/yyyy HH:mm:ss PP" type="text" class="form-control" id="proposed_act_dt" value="{{old('proposed_act_dt')}}"></input>
                                <span class="input-group-addon add-on">
                                  <i data-time-icon="fa fa-clock-o" data-date-icon="fa fa-calendar">
                                  </i>
                                </span>
                        ...
                        <div class="form-group {{ $errors->has('error_done_by') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <select name="error_done_by" class="form-control">
                                  <option value="">Select</option>
                                  @foreach(App\Model\UserAccess::all() as $access)
                                    <option value="{{$access->id}}" {{($access->id == $recId->error_done_by || $errType->id == old('error_done_by')?'selected':'')}}>{{$access->name}}</option>
                                  @endforeach  
                                </select>
