...
<div class="container">
        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    @endif
                    <form class="form-horizontal" role="form" method="POST" action="{{ route('auditing.update',$audId->id) }}" id="auditing">
                   
                        {{ csrf_field() }}

                        {{ method_field('PATCH') }}

                        <input type="hidden" name="publishing_id" value="{{$audId->publishing_id or ''}}">
                        ...
                        <div class="form-group{{ $errors->has('request_no') ? ' has-error' : '' }}">
                         ...
                         <div class="form-group{{ $errors->has('cust_name') ? ' has-error' : '' }}">
                            <label for="cust_name" class="col-md-4 control-label">Account Name</label>

                            <div class="col-md-6">
                                <input id="name" type="text" class="form-control" name="cust_name" value="{{ $indexing->cust_name or old('cust_name') }}" style="text-transform: uppercase;" onkeydown="upperCaseF(this);">
                         ...
                         <div class="form-group {{ $errors->has('publish_by') ? ' has-error' : '' }}">
                            <label for="publish_by" class="col-md-4 control-label">Publisher Name</label>
                            <div class="col-md-6">
                                <select name="publish_by" class="form-control">
                                  <option value="">Select</option>
                                  @foreach(App\Model\UserAccess::all() as $access)
                                    <option value="{{$access->id}}" {{($access->id == $audId->publish_by ||  $access->id == old('audit_by')?'selected':'')}}>{{$access->name}}</option>
                                  @endforeach  
                                </select>
                                @if ($errors->has('publish_by'))
                                    <span class="help-block">
                                        <strong>{{ $errors->first('publish_by') }}</strong>
                                    </span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="form-group {{ $errors->has('audit_by') ? ' has-error' : '' }}">
                            <label for="audit_by" class="col-md-4 control-label">Auditor Name</label>
                            <div class="col-md-6">
                                <select name="audit_by" class="form-control">
                                  <option value="">Select</option>
                                  @foreach(App\Model\UserAccess::all() as $access)
                                    <option value="{{$access->id}}" {{($access->id == Session::get('userId') ||  $access->id == old('audit_by')?'selected':'')}}>{{$access->name}}</option>
                                  @endforeach  
                                </select>
                                @if ($errors->has('audit_by'))
                                    <span class="help-block">
                                        <strong>{{ $errors->first('audit_by') }}</strong>
                                    </span>
                                @endif
                            </div>
                        </div>
 ...
 -->
                            ...
                            <div class="col-md-6">
                                <select name="status_id" class="form-control">
                                  ...
                                  @foreach(App\Model\Status::all() as $status)
                                    <option value="{{$status->id}}" {{($status->id == old('status_id') || $status->id == $audId->status_id ?'selected':'')}} >{{$status->status_name}}</option>
                                  @endforeach  
                                </select>
                            ...
                            <div class="col-md-6">
                                <select name="rfiType_id" class="form-control">
                                  ...
                                  @foreach(App\Model\RfiType::all() as $rfi)
                                    <option value="{{$rfi->id}}" {{($rfi->id == $audId->rfiType_id || $rfi->id == old('rfiType_id')?'selected':'')}} >{{$rfi->rfiType_name}}</option>
                                  @endforeach  
                                </select>
                                @if ($errors->has('region_id'))
                                    <span class="help-block">
                        ...
                        <div class="form-group{{ $errors->has('rfi_description') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <textarea id="rfi_description" class="form-control" name="rfi_description">{{$audId->rfi_description or old('rfi_description')}}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('query_raised_by') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <select name="query_raised_by" class="form-control">
                                  <option value="">Select</option>
                                  @foreach(App\Model\UserAccess::all() as $access)
                                    <option value="{{$access->id}}" {{($audId->query_raised_by == $access->id || $rfi->id == old('query_raised_by')?'selected':'')}}>{{$access->name}}</option>
                                  @endforeach  
                                </select>
                    ...
                    <div class="form-group{{ $errors->has('markError') ? ' has-error' : '' }}">
                            <label for="markError" class="col-md-4 control-label">Want to Mark as Error</label>
                            <div class="col-md-6">
                                <input type="checkbox" value="1" name="markError" {{ ($audId->markError == '1' || old('markError') == '1' ? 'checked':'')}} id="markError">
                                @if ($errors->has('markError'))
                                    <span class="help-block">
                                        <strong>{{ $errors->first('markError') }}</strong>
                                    </span>
                                @endif
                            </div>
                    </div>

                    <div id="correctionBlock"  @if($indexing->requestType->name !== 'COR') style="display:none" @endif><!-- if Indexing is COR then correction ====== this block starts here -->
                            ...
                            <div class="col-md-6">
                                <select name="error_cat_id" class="form-control">
                                  ...
                                  @foreach(App\Model\ErrorCat::all() as $errCat)
                                    <option value="{{$errCat->id}}" {{($errCat->id == $audId->error_cat_id || $errCat->id == old('error_cat_id')?'selected':'')}} >{{$errCat->name}}</option>
                                  @endforeach  
                                </select>
                            ...
                            <div class="col-md-6">
                                <select name="error_type_id" class="form-control">
                                  ...
                                  @foreach(App\Model\ErrorType::all() as $errType)
                                    <option value="{{$errType->id}}" {{($errType->id == $audId->error_type_id || $errType->id == old('error_type_id')?'selected':'')}}>{{$errType->name}}</option>
                                  @endforeach  
                                </select>
                        ...
                        <div class="form-group{{ $errors->has('root_cause') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <textarea id="root_cause" class="form-control" name="root_cause">{{$audId->root_cause or old('root_cause')}}</textarea>
                        ...
                        <div class="form-group{{ $errors->has('correction') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <textarea id="correction" class="form-control" name="correction">{{$audId->correction or old('correction')}}</textarea>
                        ...
                        <div class="form-group{{ $errors->has('corrective_action') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <textarea id="corrective_action" class="form-control" name="corrective_action">{{$audId->corrective_action or old('corrective_action')}}</textarea>
                         ...
                         <div class="form-group{{ $errors->has('preventive_action') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <textarea id="preventive_action" class="form-control" name="preventive_action">{{$audId->preventive_action or old('preventive_action')}}</textarea>
                        ...
                        <div class="form-group{{ $errors->has('proposed_comp_dt') ? ' has-error' : '' }}">
                          ...
                          <div class="col-md-6">
                              <div class="input-group date">
                                <input name="proposed_comp_dt" data-format="MM/dd/yyyy HH:mm:ss PP" type="text" class="form-control" id="proposed_comp_dt" value="{{$audId->proposed_comp_dt or old('proposed_comp_dt')}}"></input>
                                <span class="input-group-addon add-on">
                                  <i data-time-icon="fa fa-clock-o" data-date-icon="fa fa-calendar">
                                  </i>
                                </span>
                        ...
                        <div class="form-group{{ $errors->has('proposed_act_dt') ? ' has-error' : '' }}">
                          ...
                          <div class="col-md-6">
                              <div class="input-group date">
                                <input name="proposed_act_dt" data-format="MM/dd/yyyy HH:mm:ss PP" type="text" class="form-control" id="proposed_act_dt" value="{{$audId->proposed_act_dt or old('proposed_act_dt')}}"></input>
                                <span class="input-group-addon add-on">
                                  <i data-time-icon="fa fa-clock-o" data-date-icon="fa fa-calendar">
                                  </i>
                                </span>
                        ...
                        <div class="form-group {{ $errors->has('error_done_by') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <select name="error_done_by" class="form-control">
                                  <option value="">Select</option>
                                  @foreach(App\Model\UserAccess::all() as $access)
                                    <option value="{{$access->id}}" {{($access->id == $audId->error_done_by || $errType->id == old('error_done_by')?'selected':'')}}>{{$access->name}}</option>
                                  @endforeach  
                                </select>
                    ...
                    <div class="form-group{{ $errors->has('oot') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="checkbox" value="1" name="oot" {{ ($audId->oot == '1' || old('oot') == '1' ? 'checked':'')}} >
                        ...
                        <div class="form-group{{ $errors->has('oot_remarkcomment') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input id="oot_remark" type="type" class="form-control" name="oot_remark" value="{{$audId->oot_remark or old('oot_remark') }}">
                        ...
                        <div class="form-group{{ $errors->has('comment') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input id="ootcomment_commentremark" type="type" class="form-control" name="comment" value="{{$audId->comment or old('comment') }}">
                        ...
                        <div class="form-group">
                            <div class="col-md-6 col-md-offset-4">
                                ...
                                <a href="{{ route('auditing.index') }}" class="btn btn-primary">Back</a>
                            </div>
                        </div>
                    </form>
                </div>
