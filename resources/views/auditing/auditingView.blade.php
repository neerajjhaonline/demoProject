                        ...
                        <table class="table table-responsive table-striped table-bordered table-hover no-margin" id="auditing_queue" >
                         ...
                         ?>
                           <tr @if($tat['h'] <= 3) class="danger" @endif>
                             ...
                             <td>
                                @if(!$auditing->auditingId && $auditing->processQueueID)
                                  <a href="{{route('auditing.show',$auditing->processQueueID)}}" class="btn btn-xs btn-danger">{{$auditing->request_no or 'NA'}}</a>
                                @elseif($auditing->auditingId && (Session::get('userAccess') === 'admin' || Session::get('userAccess') === 'superadmin'))
                                  <a href="{{route('auditing.edit',$auditing->auditingId)}}" class="btn btn-xs btn-danger">{{$auditing->request_no or 'NA'}}</a>
                                @else
