        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    @endif
                    <form class="form-horizontal" role="form" method="POST" action="{{ route('indexing.store') }}" autocomplete="off" id="indexing">
                        {{ csrf_field() }}
                        ...
                        <div class="form-group{{ $errors->has('mail_received_dt') ? ' has-error' : '' }}">
                          ...
                          <div class="col-md-6">
                              <div id="datetimepicker2" class="input-group date">
                                <input name="mail_received_dt" data-format="MM/dd/yyyy HH:mm:ss PP" type="text" class="form-control" id="mail_received_dt" data-url="{{url('reqNum')}}"></input>
                        ...
                        <div class="form-group{{ $errors->has('request_no') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                            <input id="request_no" type="text" class="form-control" name="request_no" value="{{ old('request_no') }}" readonly>
                             ...
                             <div class="col-md-2">
                             <a href="" class="form-control btn btn-primary" id="generateNumber">Generate</a>
                            </div>
                        </div>
                        ...
                        <div class="form-group{{ $errors->has('cust_name') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input id="name" type="text" class="form-control" name="cust_name" value="{{ old('cust_name') }}" style="text-transform: uppercase;" onkeydown="upperCaseF(this);">
                            ...
                            <div class="col-md-6">
                                <select name="priority_id" class="form-control">
                                  ...
                                  @foreach(App\Model\PriorityType::all() as $pt)
                                    <option value="{{$pt->id}}" {{($pt->id == old('priority_id')?'selected':'')}}>{{$pt->name}}</option>
                                  @endforeach  
                                </select>
                            ...
                            <div class="col-md-6">
                                <select name="region_id" class="form-control">
                                  ...
                                  @foreach(App\Model\Region::all() as $region)
                                    <option value="{{$region->id}}" {{($region->id == old('region_id')?'selected':'')}} >{{$region->region_abbr}}</option>
                                  @endforeach  
                                </select>
                            ...
                            <div class="col-md-6">
                                <select name="req_type_id" class="form-control">
                                  ...
                                  @foreach(App\Model\RequestType::all() as $rq)
                                    <option value="{{$rq->id}}" {{($rq->id == old('req_type_id')?'selected':'')}}>{{$rq->name}}</option>
                                  @endforeach  
                                </select>
                            ...
                            <div class="col-md-6">
                                <select name="cont_type_id" class="form-control">
                                  ...
                                  @foreach(App\Model\ContainerType::all() as $ct)
                                    <option value="{{$ct->id}}" {{($ct->id == old('cont_type_id')?'selected':'')}}>{{$ct->name}}</option>
                                  @endforeach  
                                </select>
                            ...
                            <div class="col-md-6">
                                <select name="office_id" class="form-control">
                                  ...
                                  @foreach(App\Model\Office::all() as $off)
                                    <option value="{{$off->id}}" {{($off->id == old('office_id')?'selected':'')}}>{{$off->office_name}}</option>
                                  @endforeach  
                                </select>
